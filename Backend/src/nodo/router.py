...
def create_alerta(alerta: schemas.AlertaCreate, request: Request,current_user: str = Depends(jwt.get_current_user), db: Session = Depends(get_db)):
    return services.crear_alerta(db, alerta)

@router.get("/leer_alertas", response_model=schemas.Alerta)
def read_alerta(request: Request,current_user: str = Depends(jwt.get_current_user), db: Session = Depends(get_db)):
    return services.leer_alertas(db)

@router.get("/leer_alerta/{id_alerta}", response_model=schemas.Alerta)
def read_alerta(id_alerta: int, request: Request,current_user: str = Depends(jwt.get_current_user), db: Session = Depends(get_db)):
    return services.leer_alerta(db, id_alerta)

@router.delete("/eliminar_alerta/{id_alerta}", response_model=schemas.Alerta)
def delete_alerta(id_alerta: int, request: Request,current_user: str = Depends(jwt.get_current_user), db: Session = Depends(get_db)):
    return services.eliminar_alerta(db, id_alerta)

@router.put("/toggle_leida/{alerta_id}", response_model=schemas.Alerta)
def toggle_estado_nodo(alerta_id: int, request: Request,current_user: str = Depends(jwt.get_current_user), db: Session = Depends(get_db)):
